  ...
  if (histEl) {
    if (history.length === 0) {
      ...
      for (var i = 0; i < history.length; i++) {
        ...
        var hEngName = (p.engine || 'taker').toLowerCase().replace(/[^a-z_]/g, '');
        var hEngLabel = hEngName === 'maker' ? 'MKR' : hEngName === 'whale' ? 'WHALE' : hEngName === 'taker' ? 'TAKER' : hEngName === 'killshot' ? 'KILL' : hEngName.toUpperCase();
        var hEngClass = hEngName;
        html += '<tr>';
...
function renderEngineComparison(data) {
  ...
  var order = ['maker', 'whale', 'taker', 'killshot'];
  var rows = '';
...
/* ═══════════════════════════════════════════════════════════════
   Sub-tab switching for redesigned tabs
   ═══════════════════════════════════════════════════════════════ */

function _switchSub(agent, sub) {
  document.querySelectorAll('.' + agent + '-subtab').forEach(function(b) {
    b.classList.toggle('active', b.getAttribute('data-sub') === sub);
  });
  document.querySelectorAll('.' + agent + '-panel').forEach(function(p) {
    p.style.display = p.id === agent + '-panel-' + sub ? '' : 'none';
  });
}

function atlasSwitchSub(sub) { _switchSub('atlas', sub); }
function sorenSwitchSub(sub) { _switchSub('soren', sub); }
function lisaSwitchSub(sub)  { _switchSub('lisa', sub); }
function viperSwitchSub(sub) { _switchSub('viper', sub); }
function quantSwitchSub(sub) { _switchSub('quant', sub); }

function atlasRefreshAll() {
  if (typeof loadAtlasKBHealth === 'function') loadAtlasKBHealth();
  if (typeof loadAtlasPriorityQueue === 'function') loadAtlasPriorityQueue();
  var el = document.getElementById('atlas-updated');
  if (el) el.textContent = new Date().toLocaleTimeString();
}

function lisaRefreshAll() {
