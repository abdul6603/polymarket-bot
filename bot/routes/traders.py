...
def _normalize_garves(data: dict | None) -> list[dict]:
...
def _normalize_hawk(data: dict | None) -> list[dict]:
...
def _normalize_odin(data) -> list[dict]:
    ...
    for p in data:
        ...
        positions.append({
            "id": f"odin_{p.get('trade_id', symbol)}",
            "agent": "odin",
...
def _normalize_oracle(data) -> list[dict]:
...
def api_traders_positions():
    ...
    garves_pos = _normalize_garves(raw.get("garves"))
    hawk_pos = _normalize_hawk(raw.get("hawk"))
    odin_pos = _normalize_odin(raw.get("odin"))
    oracle_pos = _normalize_oracle(raw.get("oracle"))
