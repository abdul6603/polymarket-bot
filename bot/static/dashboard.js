...
function renderAgentGrid(overview) {
  ...
  var brainAgentMap = {garves:'garves',soren:'soren',shelby:'shelby',atlas:'atlas',lisa:'lisa',sentinel:'robotox',thor:'thor',hawk:'hawk',viper:'viper',quant:'quant',razor:'razor'};
...
var _intelAgentMap = {garves:'garves',soren:'soren',shelby:'shelby',atlas:'atlas',lisa:'lisa',sentinel:'robotox',thor:'thor',hawk:'hawk',viper:'viper',quant:'quant',razor:'razor'};
var _intelColors = {garves:'#00d4ff',soren:'#cc66ff',shelby:'#ffaa00',atlas:'#22aa44',lisa:'#ff8800',robotox:'#00ff44',thor:'#ff6600',hawk:'#FFD700',viper:'#00ff88',quant:'#00BFFF',razor:'#FF1493'};
...
async function loadOdinTab() {
  var d = await safeFetch('/api/odin');
  if (!d) return;

  // Mode label
  var modeEl = document.getElementById('odin-mode-label');
  if (modeEl) {
    var mode = (d.mode || 'offline').toUpperCase();
    modeEl.textContent = mode;
    modeEl.style.color = mode === 'LIVE' ? '#00ff44' : mode === 'PAPER' ? '#e94560' : '#888';
  }

  // Hero cards
  var bal = d.balance || 0;
  var pnl = d.total_pnl || 0;
  setText('odin-balance', '$' + bal.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2}));
  var pnlEl = document.getElementById('odin-pnl');
  if (pnlEl) {
    pnlEl.textContent = (pnl >= 0 ? '+$' : '-$') + Math.abs(pnl).toFixed(2);
    pnlEl.style.color = pnl >= 0 ? 'var(--success)' : 'var(--error)';
  }
  setText('odin-drawdown', (d.drawdown_pct || 0).toFixed(1) + '%');
  var ddEl = document.getElementById('odin-drawdown');
  if (ddEl) ddEl.style.color = (d.drawdown_pct || 0) > 10 ? 'var(--error)' : (d.drawdown_pct || 0) > 5 ? 'var(--warning)' : 'var(--success)';
  setText('odin-positions', d.open_positions || 0);

  // Macro panel
  var m = d.macro || {};
  setText('odin-macro-regime', (m.regime || '--').toUpperCase());
  var regEl = document.getElementById('odin-macro-regime');
  if (regEl) {
    var rc = m.regime === 'strong_bull' || m.regime === 'bull' ? 'var(--success)' : m.regime === 'crisis' ? 'var(--error)' : m.regime === 'bear' ? 'var(--warning)' : 'var(--text-secondary)';
    regEl.style.color = rc;
  }
  setText('odin-macro-score', m.score || '--');
  setText('odin-macro-mult', (m.multiplier || 0).toFixed(2) + 'x');
  setText('odin-spy', m.spy ? '$' + m.spy.toFixed(0) : '--');
  setText('odin-vix', m.vix ? m.vix.toFixed(1) : '--');
  setText('odin-btcd', m.btc_d ? m.btc_d.toFixed(1) + '%' : '--');
  setText('odin-usdtd', m.usdt_d ? m.usdt_d.toFixed(2) + '%' : '--');

  // Circuit breaker
  var cbEl = document.getElementById('odin-cb-status');
  if (cbEl) {
    cbEl.textContent = d.trading_allowed ? 'ALLOWED' : 'BLOCKED';
    cbEl.style.color = d.trading_allowed ? '#00ff44' : 'var(--error)';
  }
  setText('odin-consec-losses', d.consecutive_losses || 0);
  setText('odin-size-mod', ((d.size_modifier || 1) * 100).toFixed(0) + '%');
  var dpEl = document.getElementById('odin-daily-pnl');
  if (dpEl) {
  ...
  if (wpEl) {
    var wp = d.weekly_pnl || 0;
    wpEl.textContent = (wp >= 0 ? '+$' : '-$') + Math.abs(wp).toFixed(2);
    wpEl.style.color = wp >= 0 ? 'var(--success)' : 'var(--error)';
  }
  var cbReason = document.getElementById('odin-cb-reason');
  if (cbReason) cbReason.textContent = d.cb_reason || '';

  // Stats grid
  setText('odin-exposure', '$' + (d.total_exposure || 0).toFixed(2));
  setText('odin-cycles', d.cycle_count || 0);
  setText('odin-uptime', (d.uptime_hours || 0).toFixed(1) + 'h');
  var cfg = d.config || {};
  setText('odin-leverage', (cfg.leverage || 3) + 'x');
  setText('odin-risk-pct', (cfg.risk_per_trade || 3.25).toFixed(2) + '%');
  setText('odin-symbols', (d.symbols || []).join(', '));
  ...
  if (sigEl) {
    ...
    if (sig) {
      var sc = sig.direction === 'LONG' ? '#00ff44' : '#ff4444';
      var html = '<div style="display:flex;gap:16px;flex-wrap:wrap;margin-bottom:8px;">';
      html += '<span style="font-weight:700;color:' + sc + ';">' + esc(sig.direction) + '</span>';
      html += '<span>' + esc(sig.symbol) + '</span>';
      html += '<span>Conf: ' + (sig.effective_confidence || sig.confidence || 0).toFixed(2) + '</span>';
      html += '<span>R:R ' + (sig.risk_reward || 0).toFixed(1) + '</span>';
      html += '<span>Tradeable: ' + (sig.tradeable ? 'YES' : 'NO') + '</span>';
      html += '</div>';
      html += '<div style="margin-bottom:4px;">Entry: $' + (sig.entry_price || 0).toFixed(2) + ' | SL: $' + (sig.stop_loss || 0).toFixed(2) + ' | TP: $' + (sig.take_profit_1 || 0).toFixed(2) + '</div>';
      html += '<div style="margin-bottom:4px;">Macro: ' + esc(sig.macro_regime) + ' (score ' + (sig.macro_score || 0) + ', mult ' + (sig.macro_multiplier || 0).toFixed(2) + ')</div>';
      if (sig.reasons && sig.reasons.length) {
        html += '<div style="color:var(--text-muted);font-size:0.68rem;">' + sig.reasons.map(esc).join(' | ') + '</div>';
      }
      sigEl.innerHTML = html;
    } else {
      sigEl.innerHTML = '<span class="text-muted">Waiting for signal...</span>';
    }
  }

  // Paper positions
  var posEl = document.getElementById('odin-positions-table');
  if (posEl) {
    var pp = d.paper_positions || [];
    if (pp.length === 0) {
      posEl.innerHTML = '<span class="text-muted" style="font-size:0.74rem;">No open positions</span>';
    } else {
      var h = '<table style="width:100%;font-size:0.72rem;border-collapse:collapse;"><thead><tr style="color:var(--text-muted);border-bottom:1px solid rgba(255,255,255,0.06);">';
      h += '<th style="text-align:left;padding:4px;">Symbol</th><th>Dir</th><th>Entry</th><th>SL</th><th>TP</th><th>Qty</th><th>Notional</th><th>Conf</th></tr></thead><tbody>';
      for (var i = 0; i < pp.length; i++) {
        var p = pp[i];
        var dc = p.direction === 'LONG' ? '#00ff44' : '#ff4444';
        h += '<tr style="border-bottom:1px solid rgba(255,255,255,0.03);">';
        h += '<td style="padding:4px;font-weight:600;">' + esc(p.symbol) + '</td>';
        h += '<td style="color:' + dc + ';text-align:center;">' + esc(p.direction) + '</td>';
        h += '<td style="text-align:center;">$' + (p.entry_price || 0).toFixed(2) + '</td>';
        h += '<td style="text-align:center;">$' + (p.stop_loss || 0).toFixed(2) + '</td>';
        h += '<td style="text-align:center;">$' + (p.take_profit_1 || 0).toFixed(2) + '</td>';
        h += '<td style="text-align:center;">' + (p.qty || 0).toFixed(6) + '</td>';
        h += '<td style="text-align:center;">$' + (p.notional || 0).toFixed(2) + '</td>';
        h += '<td style="text-align:center;">' + (p.confidence || 0).toFixed(2) + '</td>';
        h += '</tr>';
      }
      h += '</tbody></table>';
      posEl.innerHTML = h;
...
async function loadOdinTrades() {
  var d = await safeFetch('/api/odin/trades');
  if (!d) return;
  setText('odin-total-trades', d.total || 0);
  var wrEl = document.getElementById('odin-win-rate');
  if (wrEl && d.total > 0) {
    wrEl.textContent = d.win_rate.toFixed(1) + '%';
    wrEl.style.color = wrColor(d.win_rate);
  }
  var totalFees = 0;
  var trades = d.trades || [];
  for (var i = 0; i < trades.length; i++) totalFees += (trades[i].fees || 0);
  setText('odin-total-fees', '$' + totalFees.toFixed(2));

  var el = document.getElementById('odin-trades-table');
  if (!el || trades.length === 0) return;
  var h = '<table style="width:100%;font-size:0.7rem;border-collapse:collapse;"><thead><tr style="color:var(--text-muted);border-bottom:1px solid rgba(255,255,255,0.06);">';
  h += '<th style="text-align:left;padding:4px;">Symbol</th><th>Side</th><th>Entry</th><th>Exit</th><th>P&L</th><th>Reason</th><th>Hold</th></tr></thead><tbody>';
  for (var i = 0; i < Math.min(trades.length, 30); i++) {
    var t = trades[i];
    var tpnl = t.pnl_usd || t.pnl || 0;
    var pc = tpnl >= 0 ? 'var(--success)' : 'var(--error)';
    var dc = (t.side || t.direction || '') === 'LONG' ? '#00ff44' : '#ff4444';
    h += '<tr style="border-bottom:1px solid rgba(255,255,255,0.03);">';
    h += '<td style="padding:4px;">' + esc(t.symbol) + '</td>';
    h += '<td style="color:' + dc + ';text-align:center;">' + esc(t.side || t.direction) + '</td>';
    h += '<td style="text-align:center;">$' + (t.entry_price || 0).toFixed(2) + '</td>';
    h += '<td style="text-align:center;">$' + (t.exit_price || 0).toFixed(2) + '</td>';
    h += '<td style="text-align:center;color:' + pc + ';">' + (tpnl >= 0 ? '+' : '') + tpnl.toFixed(2) + '</td>';
    h += '<td style="text-align:center;">' + esc(t.exit_reason || '--') + '</td>';
    h += '<td style="text-align:center;">' + (t.hold_hours || t.hold_duration_hours || 0).toFixed(1) + 'h</td>';
    h += '</tr>';
  }
  h += '</tbody></table>';
  el.innerHTML = h;
